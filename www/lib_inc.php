<?
	...
	function page_listing2($cur_page, $total_page, $url, $link_id="") {
		$retValue = " <nav aria-label=\"Page navigation\"><ul class=\"pagination mt-5\">";
		if($cur_page > 1) {
			$retValue .= "<li class=\"page-item\"><a class=\"page-link\" aria-label=\"이전\" href=\"".$url.($cur_page-1).$link_id."\"><i class=\"bi bi-chevron-left\"></i></a></li>";
		} else {
			$retValue .= "<li class=\"page-item\"><a class=\"page-link\" style='cursor: pointer;'><i class=\"bi bi-chevron-left\"></i></a></li>";
		}
		$start_page = ( ( (int)( ($cur_page - 1 ) / 5 ) ) * 5 ) + 1;
		...
		if($cur_page < $total_page && $total_page > 1) {
			$retValue .= "<li class=\"page-item\"><a class=\"page-link\" aria-label=\"다음\" href=\"".$url.($cur_page+1).$link_id."\"><i class=\"bi bi-chevron-right\"></i></a></li>";
		} else {
			$retValue .= "<li class=\"page-item\"><a class=\"page-link\" tabindex=\"-1\" aria-disabled=\"true\" style='cursor: pointer;'><i class=\"bi bi-chevron-right\"></i></a></li>";
		}
		$retValue .= "</ul></nav>";
...
//		}
	...
	function get_mt_code() {
		global $DB;

		$unique = false;
		do {
			$uid = substr("M".date("ymdHis", time()).strtoupper(md5(mt_rand())), 0, 16);
			$query = "select * from membership_t where mt_code = '".$uid."'";
			$cnt = $DB->count_query($query);
			if ($cnt < 1) {
				$unique = true;
				break;
	...
	function get_nationality() {
		...
		curl_setopt($curl_handle, CURLOPT_URL,'http://country.io/names.json');
		curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
		curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl_handle, CURLOPT_USERAGENT, 'effect korea');
		$codes = curl_exec($curl_handle);
		$codes_arr = json_decode($codes, true);
		curl_close($curl_handle);
		return $codes_arr;
	}

	//국제번호 리스트
	function get_international_num() {
		$curl_handle=curl_init();
		curl_setopt($curl_handle, CURLOPT_URL,'http://country.io/phone.json');
		curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
		curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl_handle, CURLOPT_USERAGENT, 'effect korea');
		$phone = curl_exec($curl_handle);
		$phone_arr = json_decode($phone, true);
		curl_close($curl_handle);
		return $phone_arr;
	}
