	...
	$(function () {
		...
		function addActiveClass(element) {
			if (current === "") {
				...
				// 	if(current!='product_list.php'){
				// 		element.parents('.nav-item').last().addClass('active');
				// 		if (element.parents('.sub-menu').length) {
				// 			element.closest('.collapse').addClass('show');
				// 			if(current!='contents_category.php'){
				// 				element.addClass('active');
				// 			}
				// 		}
...
function f_retire_mem(mt_idx) {
	if(confirm("정말로 탈퇴처리하겠습니까?")) {
		$.post('./member_update.php', {act: 'retire', idx: mt_idx}, function (data) {
...
function preview_image_sigle_selected(e) {
	...
	filesArr.forEach(function(f) {
		...
		reader.onload = function(e) {
			...
			$("#"+target_id+"_on").val(e.target.result)
		}
...
function f_update_product_info(form_act, pt_idx) {
	$.post('./product_update.php', {act: 'update_info', pt_idx: pt_idx}, function (data) {
		...
		if(product_t.pt_idx) {
			...
			f_contents_selected(product_t.ct_id);
			...
			if(product_t.pt_stock_chk == "Y") {
				$("#jaego").prop("checked", true);
				$("#pt_stock").prop('readonly', false);
				$("#pt_stock").val(product_t.pt_stock);
			} else {
			...
			$("#pt_image1_box").html('<img src="'+product_t.pt_image1_url+'?v='+date.getTime()+'" style="width: 100%;height: 100%;border-radius: 10px;"/><i class="mdi mdi-close" style="position: relative;left: 40px;top: -123px;color:red;cursor: pointer;" onclick="del_img(\'#pt_image1_box\', 1)"></i>');

			var length = $("#pt_image1_box").find("img").length;
			if(length > 0) {
				$("#pt_image1_box").attr("for", "");
			}
			...
			for(var i=2;i<=10;i++) {
				...
				if(pt_image_url_chk) {
					...
					$('#pt_image'+i+'_box').html('<img src="'+pt_image_url_chk+'?v='+date.getTime()+'" style="width: 100%;height: 100%;border-radius: 10px;" /><i class="mdi mdi-close" style="position: relative;left: 40px;top: -123px;color:red;cursor: pointer;" onclick="del_img(\'#pt_image'+i+'_box\', \''+i+'\')"></i>');

					var length = $('#pt_image'+i+'_box').find("img").length;
					if(length > 0) {
						$('#pt_image'+i+'_box').attr("for", "");
					}
				}
			}
			...
			if(product_t.pt_delivery_chk == "Y") {
				$("#new_delivery").prop("checked", true);
			} else {
				$("#basic_delivery").prop("checked", true);
			}
			if(product_t.pt_delivery_free_chk == "Y") {
				$("#pt_delivery_free_chk").prop("checked", true);
				$("#pt_delivery_free_price").attr("readonly", false);
			} else {
				$("#pt_delivery_free_chk").prop("checked", false);
				$("#pt_delivery_free_price").attr("readonly", true);
			}
			$("#pt_delivery_price").val(product_t.pt_delivery_price);
			$("#pt_delivery_free_price").val(product_t.pt_delivery_free_price);
			$("#pt_delivery_refund_price").val(product_t.pt_delivery_refund_price);
			$("#pt_delivery_exchange_price").val(product_t.pt_delivery_exchange_price);

			if(product_t.pt_random_chk == "Y") {
				$("#pt_random_chk").prop("checked", true);
				$("#effect_area").show();
			} else {
				$("#pt_random_chk").prop("checked", false);
				$("#effect_area").hide();
			}
			if(product_t.pt_random_effect == 1) {
				$("#pt_random_effect").text("(효과1)");
			} else {
				$("#pt_random_effect").text("(효과2)");
			}
		}
	});
...
function f_product_option_list(update_chk) {
	...
	$.post('./product_update.php', {
		...
		pt_option_type: $('#pt_option_type').val(),
...
function f_review_audio_content(rat_idx) {
	$.post('./review_audio_update.php', {act: 'content_view', rat_idx: rat_idx}, function (data) {
		if(data) {
			$('#modal-default-content').html(data);
			$('#modal-default').modal();
		}
...
function f_review_product_content(rpt_idx) {
	$.post('./review_product_update.php', {act: 'content_view', rpt_idx: rpt_idx}, function (data) {
		if(data) {
			$('#modal-default-content').html(data);
			$('#modal-default').modal();
		}
...
function f_pt_sale_now(chk_t) {
	if(chk_t=='Y') {
		...
		$('#f_pt_sale_now_btn3').removeClass('btn-info text-white');
	} else if(chk_t=="N") {
		...
		$('#f_pt_sale_now_btn1').removeClass('btn-info text-white');
		$('#f_pt_sale_now_btn2').removeClass('btn-info text-white');
		$('#f_pt_sale_now_btn3').addClass('btn-info text-white');
	}
...
function f_faq_content(ft_idx) {
	$.post('./faq_update.php', {act: 'content_view', ft_idx: ft_idx}, function (data) {
		if(data) {
			$('#modal-default-content').html(data);
			$('#modal-default').modal();
		}
...
function f_push_content(ft_idx) {
	$.post('./push_update.php', {act: 'content_view', ft_idx: ft_idx}, function (data) {
		if(data) {
			$('#modal-default-content').html(data);
			$('#modal-default').modal();
		}
...
function f_report_content(rt_idx) {
	$.post('./report_update.php', {act: 'content_view', rt_idx: rt_idx}, function (data) {
		if(data) {
			$('#modal-default-content').html(data);
			$('#modal-default').modal();
		}
...
function f_select_contents(cc){
	$.post('./product_update.php', {act: 'select_contents', cc_c: cc}, function (data) {
		if(data) {
			$('#select_contents_box').html(data);
		}
	});
	return false;	
}
function f_search_contents(stxt) {
	$('#serach_contents_box').show();
	$.post('./product_update.php', {act: 'search_contents', stxt: stxt}, function (data) {
		if(data) {
			$('#serach_contents_box').html(data);
		}
	});
...
function f_contents_selected(idx) {
	$('.contents_selected').each(function(i, obj) {
		$('.contents_selected').removeClass('contents_selected_active');
	});
	$('#contents_selected_'+idx).addClass('contents_selected_active');

	$.post('./product_update.php', {act: 'contents_selectecd', idx : idx}, function (data) {
		var json_data = JSON.parse(data);

		if(json_data.title) {
			$('#selected_contents_t').html(json_data.title);
			$('#serach_contents_txt').val('');
			$('#serach_contents_box').hide();
			$('#ct_id').val(json_data.idx);									
		}
...
function f_select_product(pc){
	$.post('./product_update.php', {act: 'select_product', pc_c: pc}, function (data) {
		if(data) {
...
function f_search_product(stxt) {
	$('#serach_product_box').show();
	$.post('./product_update.php', {act: 'search_product', stxt: stxt}, function (data) {
		if(data) {
			$('#serach_product_box').html(data);
		}
	});
...
function f_product_selected(idx) {
	$('.product_selected').each(function(i, obj) {
		$('.product_selected').removeClass('product_selected_active');
	});
	$('#product_selected_'+idx).addClass('product_selected_active');

	$.post('./product_update.php', {act: 'product_selectecd', idx : idx}, function (data) {
		var json_data = JSON.parse(data);

		if(json_data.title) {
			$('#selected_product_t').html(json_data.title);
			$('#serach_product_txt').val('');
			$('#serach_product_box').hide();
			$('#pt_id').val(json_data.idx);									
		}
...
function f_mt_approve(chk_t) {
	if(chk_t=='') {
		$('#f_mt_approve_btn1').addClass('btn-info text-white');
		$('#f_mt_approve_btn2').removeClass('btn-info text-white');
		$('#f_mt_approve_btn3').removeClass('btn-info text-white');
		$('#f_mt_approve_btn4').removeClass('btn-info text-white');
	} else if(chk_t=="1") {
		$('#f_mt_approve_btn1').removeClass('btn-info text-white');
		$('#f_mt_approve_btn2').addClass('btn-info text-white');
		$('#f_mt_approve_btn3').removeClass('btn-info text-white');
		$('#f_mt_approve_btn4').removeClass('btn-info text-white');
	} else if(chk_t=="2") {
		$('#f_mt_approve_btn1').removeClass('btn-info text-white');
		$('#f_mt_approve_btn2').removeClass('btn-info text-white');
		$('#f_mt_approve_btn3').addClass('btn-info text-white');
		$('#f_mt_approve_btn4').removeClass('btn-info text-white');
	} else {
		$('#f_mt_approve_btn1').removeClass('btn-info text-white');
		$('#f_mt_approve_btn2').removeClass('btn-info text-white');
		$('#f_mt_approve_btn3').removeClass('btn-info text-white');
		$('#f_mt_approve_btn4').addClass('btn-info text-white');
	}
	$('#mt_approve').val(chk_t);

	return false;
}

function f_ct_type(chk_t) {
	if(chk_t=='') {
		$('#f_ct_type_btn1').addClass('btn-info text-white');
		$('#f_ct_type_btn2').removeClass('btn-info text-white');
		$('#f_ct_type_btn3').removeClass('btn-info text-white');
	} else if(chk_t=="1") {
		$('#f_ct_type_btn1').removeClass('btn-info text-white');
		$('#f_ct_type_btn2').addClass('btn-info text-white');
		$('#f_ct_type_btn3').removeClass('btn-info text-white');
	} else if(chk_t=="2") {
		$('#f_ct_type_btn1').removeClass('btn-info text-white');
		$('#f_ct_type_btn2').removeClass('btn-info text-white');
		$('#f_ct_type_btn3').addClass('btn-info text-white');
	}
	$('#ct_type').val(chk_t);
...
function f_ct_resolution(chk_t) {
	if(chk_t=='6K+') {
		...
		$('#f_ct_resolution_btn1').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn2').addClass('btn-info text-white');
		$('#f_ct_resolution_btn3').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn4').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn5').removeClass('btn-info text-white');
	} else if(chk_t=="QHD") {
		$('#f_ct_resolution_btn1').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn2').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn3').addClass('btn-info text-white');
		$('#f_ct_resolution_btn4').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn5').removeClass('btn-info text-white');
	} else if(chk_t=="FHD") {
		$('#f_ct_resolution_btn1').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn2').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn3').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn4').addClass('btn-info text-white');
		$('#f_ct_resolution_btn5').removeClass('btn-info text-white');
	} else {
		$('#f_ct_resolution_btn1').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn2').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn3').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn4').removeClass('btn-info text-white');
		$('#f_ct_resolution_btn5').addClass('btn-info text-white');
	}
	$('#ct_resolution').val(chk_t);
...
function f_extension(format) {
	var html = "";
	if(format == "4K" || format == "FHD" || format == "Drone" || format == "Time lapse") {
		html = '.mp4, .mov';
	} else if(format == "LUT") {
		html = '.Cube LUT, .drx, .dpx, .zip';
	} else if(format == "Transition" || format == "Motion") {
		html = '.prfpset, .mogrt, .zip';
	} else if(format == "Subtitle template") {
		html = '.mogrt, .zip';
	} else if(format == "image") {
		html = '.jpg, .jpeg, .png, .raw, .HEIF, .HEIC';
	} else if(format == "2D illustration" || format == "3D illustration") {
		html = '.ai, .eps, .svg, .psd, .indd';
	}
	return html;
}
...
function del_img2(e) {
	var id = e.id;
	$("#"+id).val("");
	$("#"+id+"_box").css('border', 'none');
	$("#"+id+"_box").html('');
}

function f_ct_show(chk_t) {
	if(chk_t=='') {
		$('#f_pt_sale_now_btn1').addClass('btn-info text-white');
		$('#f_pt_sale_now_btn2').removeClass('btn-info text-white');
		$('#f_pt_sale_now_btn3').removeClass('btn-info text-white');
