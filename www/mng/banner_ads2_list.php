...
$query = "select a1.*, a1.idx as bt_idx from banner_t a1 where bt_main = 'N' and bt_type = 2 ";
$query_count = "select count(*) from banner_t a1 where bt_main = 'N' and bt_type = 2 ";
if($_GET['bt_language']) {
    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">광고배너2관리 (아티스트 홈 페이지)</h4>
                        <div class="card-description float-right">
                            <input type="button" class="btn btn-info" value="신규등록" onclick="location.href='./banner_form.php?type=2&act=input&<?=$_get_txt.$_GET['pg']?>'"/>
                        </div>
                            ...
                            if($list) {
                                foreach($list as $row) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="media">
                                                <img src="<?=$ct_img_url.'/'.$row['bt_file']?>" class="align-self-center" style="width: 100%;height:80px;border-radius: 0;">
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <?=$row['bt_contents']?>
                                        ...
                                        <td class="text-center">
                                            <input type="button" class="btn btn-outline-dark btn-sm" value="수정" onclick="location.href='./banner_form.php?type=2&act=update&bt_idx=<?=$row['bt_idx']?>&<?=$_get_txt.$_GET['pg']?>'" />
...
<script>
    ...
    function f_language(language) {
        location.replace("./banner_ads2_list.php?bt_language="+language);
    }
</script>
<?
